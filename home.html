...
function sensor() { //ジャイロセンサーを使う関数
    window.addEventListener("deviceorientation", (dat) => {
        alpha = dat.alpha; //x軸
        beta = dat.beta; //y軸
        gamma = dat.gamma; //z軸
        console.log("ジャイロセンサー"); //確認用
    });
}

function sensor2() {　//環境光センサーを使う関数
...
setInterval('sensor2()', 1000); //環境光センサーを1秒に1回回す
...
function displayDate() {
    ...
    txt.innerHTML = "alpha:" + alpha + "<br>" + "beta:" + beta + "<br>" + "gamma:" + gamma; //ジャイロセンサーの値を入れる
